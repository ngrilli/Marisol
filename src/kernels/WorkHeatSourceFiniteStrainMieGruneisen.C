#include "WorkHeatSourceFiniteStrainMieGruneisen.h"

template <>
InputParameters
validParams<WorkHeatSourceFiniteStrainMieGruneisen>()
{
...
WorkHeatSourceFiniteStrainMieGruneisen::WorkHeatSourceFiniteStrainMieGruneisen(const InputParameters & parameters)
...
Real
WorkHeatSourceFiniteStrainMieGruneisen::computeQpResidual()
{
  Real heat_source;
...
Real
WorkHeatSourceFiniteStrainMieGruneisen::computeQpJacobian()
{
...
Real
WorkHeatSourceFiniteStrainMieGruneisen::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real val;
  ...
  if (jvar == _c_var) {
    ...
    val = - 2.0 * (1.0 - _c[_qp]) * _stress[_qp].doubleContraction(_strain_rate[_qp]) * _phi[_j][_qp] * _test[_i][_qp];
  }
