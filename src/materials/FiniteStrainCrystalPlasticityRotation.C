...
#include "FiniteStrainCrystalPlasticityRotation.h"
...
registerMooseObject("veryhappyApp", FiniteStrainCrystalPlasticityRotation);

template <>
InputParameters
validParams<FiniteStrainCrystalPlasticityRotation>()
{
...
FiniteStrainCrystalPlasticityRotation::FiniteStrainCrystalPlasticityRotation(const InputParameters & parameters)
  : ComputeStressBase(parameters),
    ...
    _gssincr(declareProperty<std::vector<Real>>("gssincr")),    // Slip system resistances incr
    ...
    _theta(declareProperty<Real>("theta")), //rotation angle
    _re(declareProperty<RankTwoTensor>("re")), //rotation tensor
    ...
    _fp1(declareProperty<std::vector<Real> >("fp1")),
    _fp2(declareProperty<std::vector<Real> >("fp2")),
    _fp3(declareProperty<std::vector<Real> >("fp3"))

/*
    _curl_fp1(_is_implicit ? _fp1.curlSln() : _fp1.curlSlnOld()),
    _curl_fp2(_is_implicit ? _fp2.curlSln() : _fp2.curlSlnOld()),
    _curl_fp3(_is_implicit ? _fp3.curlSln() : _fp3.curlSlnOld()),

    _curl_fp(declareProperty<RankTwoTensor>("curl_fp")),
    _curl_fp_old(getMaterialPropertyOld<RankTwoTensor>("curl_fp"))
*/
/*

   // _fp1_old(declarePropertyOld<std::vector<Real> >("fp1")),
   // _fp2_old(declarePropertyOld<std::vector<Real> >("fp2")),
   // _fp3_old(declarePropertyOld<std::vector<Real> >("fp3"))
 */
...
void
FiniteStrainCrystalPlasticityRotation::initQpStatefulProperties()
{
  _stress[_qp].zero();
...
void
FiniteStrainCrystalPlasticityRotation::initSlipSysProps()
{
...
void
FiniteStrainCrystalPlasticityRotation::assignSlipSysRes()
{
...
void
FiniteStrainCrystalPlasticityRotation::readFileInitSlipSysRes()
{
  _gss[_qp].resize(_nss);
...
void
FiniteStrainCrystalPlasticityRotation::getInitSlipSysRes()
{
  if (_gprops.size() <= 0)
...
void
FiniteStrainCrystalPlasticityRotation::readFileFlowRateParams()
{
  _a0.resize(_nss);
  _xm.resize(_nss);
...
void
FiniteStrainCrystalPlasticityRotation::getFlowRateParams()
{
...
void
FiniteStrainCrystalPlasticityRotation::readFileHardnessParams()
{
}

// Read hardness parameters from .i file
void
FiniteStrainCrystalPlasticityRotation::getHardnessParams()
{
  if (_hprops.size() <= 0)
...
void
FiniteStrainCrystalPlasticityRotation::getSlipSystems()
{
...
void
FiniteStrainCrystalPlasticityRotation::initAdditionalProps()
{
}
 ...
 */
void
FiniteStrainCrystalPlasticityRotation::computeQpStress()
{
...
void
FiniteStrainCrystalPlasticityRotation::preSolveQp()
{
  // Initialize variable
  if (_first_substep)
  {
...
void
FiniteStrainCrystalPlasticityRotation::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
FiniteStrainCrystalPlasticityRotation::postSolveQp()
{
  if (_err_tol)
  {
    _err_tol = false;
  ...
  else
  {
    ...
    _elastic_strain[_qp] = _fe.transpose() * _fe - iden;
    _elastic_strain[_qp] = _elastic_strain[_qp] * 0.5;
...
void
FiniteStrainCrystalPlasticityRotation::preSolveStatevar()
{
...
void
FiniteStrainCrystalPlasticityRotation::solveStatevar()
{
  Real gmax, gdiff;
  unsigned int iterg;
...
void
FiniteStrainCrystalPlasticityRotation::postSolveStatevar()
{
  if (_max_substep_iter == 1) // No substepping
  {
    _gss[_qp] = _gss_tmp;
...
void
FiniteStrainCrystalPlasticityRotation::preSolveStress()
{
  if (_max_substep_iter == 1) // No substepping
  {
...
void
FiniteStrainCrystalPlasticityRotation::solveStress()
{
  unsigned int iter = 0;
...
void
FiniteStrainCrystalPlasticityRotation::postSolveStress()
{
  if (_max_substep_iter == 1) // No substepping
  {
  ...
  /*
    _fp1[_qp][0] = _fp[_qp](0,0);
    _fp1[_qp][1] = _fp[_qp](0,1);
    _fp1[_qp][2] = _fp[_qp](0,2);
    
    _fp2[_qp][0] = _fp[_qp](1,0);
    _fp2[_qp][1] = _fp[_qp](1,1);
    _fp2[_qp][2] = _fp[_qp](1,2);
    
    _fp3[_qp][0] = _fp[_qp](2,0);
    _fp3[_qp][1] = _fp[_qp](2,1);
    _fp3[_qp][2] = _fp[_qp](2,2);
    
*/
    
   /*
  //Calculate dislocation density tensor
 
    std::vector<Real> plastic_de1(3);
    std::vector<Real> plastic_de2(3);
    std::vector<Real> plastic_de3(3);
    
    std::vector<Real> _curl_fp1(3);
    
    plastic_de1[0] = _fp[_qp](0,0);
    plastic_de1[1] = _fp[_qp](0,1);
    plastic_de1[2] = _fp[_qp](0,2);

    plastic_de2[0] = _fp[_qp](1,0);
    plastic_de2[1] = _fp[_qp](1,1);
    plastic_de2[2] = _fp[_qp](1,2);

    plastic_de3[0] = _fp[_qp](2,0);
    plastic_de3[1] = _fp[_qp](2,1);
    plastic_de3[2] = _fp[_qp](2,2);

    ncurl(coupledComponents(plastic_de1));
    
    for (unsigned int i = 0; i < ncurl; ++i)
    {
        plastic_de1[i] = &coupledValue(plastic_de1, i);
        curl_r[i] = &coupledCurl(plastic_de1, i);
    }
    
    RankTwoTensor curl_fp = (curl_r[0], curl_r[1,curl_r[2]]);
    */
  ...
  _fp1[_qp] = _fp[_qp](0,i);
  _fp2[_qp] = _fp[_qp](1,i);
  _fp3[_qp] = _fp[_qp](2,i);
    
  Real grad1 = _grad_fp1[_qp];
  Real grad2 = _grad_fp2[_qp];
  Real grad3 = _grad_fp3[_qp];
    
  TensorValue<Real> fp_vector(grad1, grad2, grad3);
  RealVectorValue fp_grad = TensorTools::curl_from_grad(fp_vector);
  
  _curl_fp(i,0) = fp_grad(0);
  _curl_fp(i,1) = fp_grad(1);
  _curl_fp(i,2) = fp_grad(2);
...
void
FiniteStrainCrystalPlasticityRotation::update_slip_system_resistance()
{
  updateGss();
}
 ...
 */
void
FiniteStrainCrystalPlasticityRotation::updateGss()
{
  DenseVector<Real> hb(_nss);
  Real qab;
    
  _gssincr[_qp].resize(_nss, 0.0);
  ...
  for (unsigned int i = 0; i < _nss; ++i)
  {
    ...
    for (unsigned int j = 0; j < _nss; ++j)
    {
      ...
      _gssincr[_qp][i] += qab * hb(j) * std::abs(_slip_incr(j));
...
void
FiniteStrainCrystalPlasticityRotation::calc_resid_jacob(RankTwoTensor & resid, RankFourTensor & jac)
{
  calcResidual(resid);
  if (_err_tol)
    return;
...
void
FiniteStrainCrystalPlasticityRotation::calcResidual(RankTwoTensor & resid)
{
  ...
  // Calculate the eigenvaule and eigenvectors for ce
  ...
  // Calculate stretch tensor Ue
  RankTwoTensor Ue = N1 * lambda1 + N2 * lambda2 + N3 * lambda3;
  RankTwoTensor invUe(Ue.inverse());
    
  // Calculate rotation tensor
  _re[_qp] = _fe * invUe;
  RankTwoTensor dre = _re[_qp] - _re[_qp].transpose();
    
  // Calculate rotation tensor
    
    if ( dre(0,1) > 0){
      _theta[_qp] = std::asin(dre.L2norm() / 2.0 / std::sqrt(2)) * 180.0 / libMesh::pi;
    }
    else
    {
      _theta[_qp] = std::asin(dre.L2norm() / 2.0 / std::sqrt(2)) * 180.0 / libMesh::pi;
...
void
FiniteStrainCrystalPlasticityRotation::calcJacobian(RankFourTensor & jac)
{
...
void
FiniteStrainCrystalPlasticityRotation::getSlipIncrements()
{
  for (unsigned int i = 0; i < _nss; ++i)
  {
    _slip_incr(i) = _a0(i) * std::pow(std::abs(_tau(i) / _gss_tmp[i]), 1.0 / _xm(i)) *
                    copysign(1.0, _tau(i)) * _dt;
    _slip_strain(i) = _a0(i) * std::pow(std::abs(_tau(i) / _gss_tmp[i]), 1.0 / _xm(i)) * copysign(1.0, _tau(i)) * _dt;
...
RankTwoTensor
FiniteStrainCrystalPlasticityRotation::get_current_rotation(const RankTwoTensor & a)
{
  return getMatRot(a);
}
...
RankTwoTensor
FiniteStrainCrystalPlasticityRotation::getMatRot(const RankTwoTensor & a)
{
  RankTwoTensor rot;
...
void
FiniteStrainCrystalPlasticityRotation::computeQpElasticityTensor()
{
}

RankFourTensor
FiniteStrainCrystalPlasticityRotation::calcTangentModuli()
{
...
void
FiniteStrainCrystalPlasticityRotation::calc_schmid_tensor()
{
...
RankFourTensor
FiniteStrainCrystalPlasticityRotation::elastoPlasticTangentModuli()
{
  RankFourTensor tan_mod;
...
RankFourTensor
FiniteStrainCrystalPlasticityRotation::elasticTangentModuli()
{
  return _elasticity_tensor[_qp]; // update jacobian_mult
}

bool
FiniteStrainCrystalPlasticityRotation::line_search_update(const Real rnorm_prev, const RankTwoTensor dpk2)
{
...
void
FiniteStrainCrystalPlasticityRotation::internalVariableUpdateNRiteration()
{
