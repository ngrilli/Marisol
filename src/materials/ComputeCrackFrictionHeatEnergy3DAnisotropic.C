...
#include "ComputeCrackFrictionHeatEnergy3DAnisotropic.h"
#include <math.h>

template <>
InputParameters
validParams<ComputeCrackFrictionHeatEnergy3DAnisotropic>()
{
  ...
  params.addClassDescription("Heat energy density generated by crack friction, according to Dienes 2006"
                             " and to Grilli 2018, using the anisotropic damage model for the"
...
ComputeCrackFrictionHeatEnergy3DAnisotropic::ComputeCrackFrictionHeatEnergy3DAnisotropic(const InputParameters & parameters)
...
void
ComputeCrackFrictionHeatEnergy3DAnisotropic::computeQpProperties()
{
  ...
  Mcoupling = _dcdx[_qp] * _cleavage_plane_normal[0]
            + _dcdy[_qp] * _cleavage_plane_normal[1]
            + _dcdz[_qp] * _cleavage_plane_normal[2];

  _crack_surface_density[_qp] = 0.5 * _c[_qp] * _c[_qp]
                              + 0.5 * _l * _l * (1.0 + _beta_penalty)
                              * _crack_normal_norm[_qp] * _crack_normal_norm[_qp]
                              - 0.5 * _l * _l * _beta_penalty * Mcoupling * Mcoupling;
  ...
  _slide_velocity_parallel[_qp] = std::sqrt(
				  std::pow((_crack_normal[_qp][1]*_slide_velocity[_qp][2]-_crack_normal[_qp][2]*_slide_velocity[_qp][1]),2)
				+ std::pow((_crack_normal[_qp][0]*_slide_velocity[_qp][2]-_crack_normal[_qp][2]*_slide_velocity[_qp][0]),2)
				+ std::pow((_crack_normal[_qp][0]*_slide_velocity[_qp][1]-_crack_normal[_qp][1]*_slide_velocity[_qp][0]),2));
